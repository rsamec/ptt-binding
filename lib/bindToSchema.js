...
var Binder = require('react-binding/lib/MobxBinder')["default"];
...
function initBindings(provider, frozenSchema, dataBinder, reaction, cursor) {
    ...
    trav(frozenSchema, function (x, cursor) {
        bindContainer(x, provider.newCursor(cursor), dataBinder, reaction);
        bindBoxes(x, provider.newCursor(cursor), dataBinder, reaction);
    }, cursor);
}
...
var bindBoxes = function (x, cursor, dataBinder, reaction) {
    ...
    for (var i = 0; i != boxes.length; i++) {
        ...
        bindProps(updated, cursor.newCursor(["boxes", i, "props"]), dataBinder, false, reaction);
    }
};
var bindContainer = function (x, cursor, dataBinder, reaction) {
    var visibilityBinding = x.bindings && x.bindings[VISIBILITY_KEY];
    if (visibilityBinding !== undefined) {
        if (!!!visibilityBinding.path)
            return;
        var binding_1 = getValueBinding(dataBinder, visibilityBinding);
    ...
    if (x.elementName === REPEATER_CONTAINER_NAME) {
        ...
        if (!!!itemsBinding.path)
            return;
        var binding_2 = getValueBinding(dataBinder, itemsBinding);
        var rowsToRemove = 1;
        var lastRange = [];
        var repeaterReaction = function (dataLength) {
            ...
            if (dataLength > currentLength) {
                ...
                repeatContainers(range, x, cursor, binding_2, reaction);
            }
            else {
    ...
    for (var i = 0; i != containers.length; i++) {
        ...
        bindProps(updated, cursor.newCursor(["containers", i, "props"]), dataBinder, false, reaction);
    }
};
var repeatContainers = function (range, x, cursor, binding, reaction) {
    var clonedRows = [];
...
var bindProps = function (box, cursor, dataBinder, isDesignMode, reaction) {
    if (box === undefined)
        return;
    ...
    var _loop_1 = function () {
        ...
        var binding = isArrayBinding ? getArrayBinding(dataBinder, bindingProps) : getValueBinding(dataBinder, bindingProps);
...
var getConverter = function (dataBinder, bindingProps) {
    var converter;
...
var getValueBinding = function (dataBinder, bindingProps) {
...
var getArrayBinding = function (dataBinder, bindingProps) {
    var converter = getConverter(dataBinder, bindingProps);
